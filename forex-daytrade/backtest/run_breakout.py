"""CLI do backtest da estratégia de rompimento do range asiático — terceiro candidato,
pesquisado depois que tendência (EMA) e reversão à média (Bollinger+RSI) não mostraram
expectância positiva consistente (ver README.md). Mesmo padrão de CLI dos outros backtests.

Uso:
    python -m backtest.run_breakout                 # 90 dias, FOREX_SYMBOLS, timeframe 1h
    python -m backtest.run_breakout --days 60
    python -m backtest.run_breakout --symbols GBP/USD
"""
...
VARIANTS = [
    BreakoutVariant(name="rompimento range asiático (baseline)"),
]
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--timeframe", type=str, default="1h", help="Intervalo dos candles (Twelve Data)")
...
def main() -> None:
    ...
    print()
    print(f"Backtest (rompimento range asiático): {len(symbols)} par(es), {args.days} dias, timeframe {args.timeframe}")
