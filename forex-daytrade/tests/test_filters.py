...
def _candles_from_closes(closes: list[float]) -> list[Candle]:
    return [
        Candle(open_time_ms=i, open=c, high=c + 1.0, low=c - 1.0, close=c, volume=0.0)
...
def test_signal_candle_quality_passes_for_strong_long_bar():
    final = Candle(open_time_ms=999, open=100.5, high=101.2, low=100.3, close=101.0, volume=0.0)
...
def test_signal_candle_quality_blocks_weak_close_for_long():
    final = Candle(open_time_ms=999, open=100.1, high=101.0, low=100.0, close=100.2, volume=0.0)
    window = _flat_window_with_final_candle(final)
...
def test_signal_candle_quality_blocks_body_not_fully_above_ema_for_long():
    final = Candle(open_time_ms=999, open=99.5, high=101.0, low=99.0, close=100.5, volume=0.0)
    window = _flat_window_with_final_candle(final)
    assert passes_signal_candle_quality_filter(window, "long", min_close_position=0.5) is False


def test_signal_candle_quality_passes_for_strong_short_bar():
    final = Candle(open_time_ms=999, open=99.5, high=99.7, low=98.8, close=99.0, volume=0.0)
    window = _flat_window_with_final_candle(final)
...
def test_signal_candle_quality_blocks_zero_range_candle():
    final = Candle(open_time_ms=999, open=101.0, high=101.0, low=101.0, close=101.0, volume=0.0)
    window = _flat_window_with_final_candle(final)
    assert passes_signal_candle_quality_filter(window, "long", min_close_position=0.5) is False
